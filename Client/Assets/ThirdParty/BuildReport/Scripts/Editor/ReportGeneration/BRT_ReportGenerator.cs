...
*/
	...
	static double GetStandaloneBuildSize(string buildFilePath)
	{
		if (Directory.Exists(buildFilePath))
		{
			...
			string[] potentialBuildExeFiles = Directory.GetFiles(buildFilePath, "*.exe", SearchOption.TopDirectoryOnly);
	...
	{
		if (IsSingleStandaloneBuildInPath(buildFilePath))
		{
			// then just get the total size of the parent folder
			
			string parentFolderPath = Path.GetDirectoryName(buildFilePath);
			
			//Debug.LogFormat("GetStandaloneBuildWithDataFolderSize: Getting size of whole folder {0}", parentFolderPath);

			double parentFolderByteSize = BuildReportTool.Util.GetPathSizeInBytes(parentFolderPath);
	...
	static bool IsSingleStandaloneBuildInPath(string buildFilePath)
	{
		...
		{
			var manyExeFiles = Directory.GetFiles(parentFolderPath, "*.exe", SearchOption.TopDirectoryOnly).Length >= 2;
			if (manyExeFiles)
			{
				//Debug.LogFormat("IsSingleStandaloneBuildInPath: Many .exe files found in {0}", parentFolderPath);
				return false;
			}

			var manyX86Files = Directory.GetFiles(parentFolderPath, "*.x86", SearchOption.TopDirectoryOnly).Length >= 2;
			if (manyX86Files)
			{
