				...
				if (rect.Contains(e.mousePosition)){
					foreach(FSMConnection connection in inConnections){
						if (connection.sourceNode == clickedPort.parent){
							Logger.LogWarning("State is already connected to target state. Consider using a 'ConditionList' on the existing transition if you want to check multiple conditions", "Editor", this);
							clickedPort = null;
							e.Use();
							return;
						}
