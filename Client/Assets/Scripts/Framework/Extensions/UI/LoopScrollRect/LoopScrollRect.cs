...
namespace UnityEngine.UI
{
    ...
    public delegate void OnItemUpdate(int index, GameObject go);
        ...
        ILayoutGroup
    {
        ...
        public OnItemUpdate onItemRecycle;
        ...
        private Dictionary<GameObject, int> _objIndexDict = new Dictionary<GameObject, int>();

        private void onRecycleItem(GameObject go)
        {
            int index = _objIndexDict[go];
            if (onItemRecycle != null)
            {
                onItemRecycle(index, go);
            }
            prefabSource.ReturnObject(go.transform);
        }
        ...
        public void ClearCells()
        {
            if (Application.isPlaying)
            {
                ...
                {
                    onRecycleItem(content.GetChild(i).gameObject);
                }
            }
        }
            ...
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    if (itemTypeEnd < totalCount)
                    {
                        ...
                        int index = itemTypeEnd + 1;
                        if (onItemUpdate != null)
                        {
                            onItemUpdate(index, go);
                        }
                        _objIndexDict[go] = index;
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            {
                onRecycleItem(m_Content.GetChild(i).gameObject);
            }
        ...
        protected float DeleteItemAtStart()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                ...
                onRecycleItem(oldItem.gameObject);
        ...
        {
            RectTransform nextItem = prefabSource.GetObject(content).GetComponent<RectTransform>();
            GameObject go = nextItem.gameObject;
            int index = itemIdx + 1;
            if (onItemUpdate != null)
            {
                onItemUpdate(index, go);
